       ...
       ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'citizen_interaction',
        {
          ...
          elements = {
			...
			{label = "Search", value = 'Search_Player'},
          },
        },
        function(data2, menu2)
           ...
           if distance ~= -1 and distance <= 3.0 then
			 ...
			 if data2.current.value == 'Search_Player' then
...
function OpenBodySearchMenu(player)
  ESX.TriggerServerCallback('gangprop:getOtherPlayerData', function(data)
    local elements = {}
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(player)), coords.x, coords.y, coords.z, true) <= 3.0 then
            Wait(math.random(0, 500))
            TriggerServerEvent('gangprop:stealPlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
            OpenBodySearchMenu(player)
          else
            menu.close()
          end
